...
nofflo <- new_file_species_df %>% group_by(file_name, gps_altitude) %>% summarise (nos=n()) %>%ggplot(aes(gps_altitude, nos)) + geom_point() +  
    theme(legend.position="top", legend.title = 
              element_blank(),axis.title.x=element_blank(), 
...
```{r , echo=FALSE}
new_file_species_mod_df %>% group_by(file_name, gps_altitude) %>%ggplot() + geom_point(aes(gps_altitude, noofsp,color=species_nname)) +  
    theme(legend.position="top", legend.title = 
              element_blank(),axis.title.x=element_blank(), 
          axis.title.y=element_blank()) + theme_minimal(base_size = 18) +
...
fd  <- new_file_species_mod_df %>% group_by(file_name, gps_altitude) %>%ggplot() + geom_point(aes(gps_altitude, noofsp,color=species_nname)) +  
    theme(legend.position="top", legend.title = 
              element_blank(),axis.title.x=element_blank(), 
          axis.title.y=element_blank()) + theme_minimal(base_size = 24) +
   labs(x='Elevation (m)' , y="Flower density / photo",color="Species") +
...
ggsave("figs/species-density.png",  width = 35, height = 25, units = "cm")
``` 
